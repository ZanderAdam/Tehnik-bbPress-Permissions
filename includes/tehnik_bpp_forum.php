<?php
	...
	function tehnik_bpp_filter_forums_by_permissions( $args = '' ){
		...
		$allowed_forums = tehnik_bpp_get_permitted_forum_ids(new WP_Query($meta_query));
	 ...
	 * The function returns an array of forum IDs which user has access to.
	 */
	function tehnik_bpp_get_permitted_forum_ids($forum_query)
	{		
		...
		while ( $forum_query->have_posts() ) : 		
			...
			if(members_can_user_view_post($user_id, $post_id))
			{
				...
				array_push($allowed_forums, $post_id);
			}
